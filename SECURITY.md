...
- **Trial gate:** Middleware blocks `/calculator` and `/history` after `companies.trial_ends_at` (UI/session layer)
...
- **Trial enforcement** is primarily middleware/UI today; not fully enforced inside every RLS policy
...
- **No offline/PWA** — requires network for charts, auth, and saves
...
- Feature requests (Stripe, PWA, invite-only) — use normal product channels
